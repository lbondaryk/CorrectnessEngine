...
//        FOR THE MEANTIME THOUGH.
    ...
    */
    ...
    /**
     * Calculate the correct answer if appropriate.  This will be the first element
     * in the codeExamples array.
     *
    ...
    this.addCorrectAnswer = function(returnData, answerKey, isLastAttempt)
    {
        ...
        if (isLastAttempt && returnData.correctness !== 1) {
            ...
            if (_.has(answerKey.answers, 'codeExamples') 
                && !_.isEmpty(answerKey.answers.codeExamples) 
                && _.has(answerKey.answers.codeExamples[0], 'code'))
            {
                returnData.correctAnswer = {};
                returnData.correctAnswer.correctCode = answerKey.answers.codeExamples[0].code;
            }
