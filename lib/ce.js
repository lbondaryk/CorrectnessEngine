...
module.exports.EngineHandler = function()
{
    ...
    this.assess = function(handler, payload, callback)
    {
        ...
        .then(function () {
            ...
            return handler.preprocess(answerKey, studentSubmission, returnData);
        })
        .then(function (returnData) {
            // Add the score and feedback to returnData
            return handler.calculateScoreAndFeedback(answerKey, studentSubmission, returnData);
        })
        .then(function (returnData) {
            // Add the correct answer to returnData if appropriate
            return handler.addCorrectAnswer(answerKey, payload.isLastAttempt, returnData);
        })
        .then(function (returnData) {
