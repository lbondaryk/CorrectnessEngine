...
module.exports.EngineHandler = function()
{
     ...
     * appropriate answerType handler.
     ...
     * 
     * @param  {Function} callback Callback with signature fn(error, results),
     *                             where results will contain correctAnswer
     */
    this.retrieveAnswer = function(answerKey, callback)
    {
        var assessmentType = answerKey.assessmentType;
        if (!(assessmentType in assessmentHandlers))
        {
            callback("RetrieveAnswer failed because the assessmentType '" + assessmentType + "' can not be processed by this Correctness Engine", null);
            return;
        }
