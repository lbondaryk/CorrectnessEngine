...
module.exports.EngineHandler = function ()
{
     ...
     *      The header and payload information for the request. Should contain
     *      the user and course Ids and the answerKey.
     *
     * @returns {Promise} A promise whose resolved value is an assessment handler
     *      instance for the request.
     */
    var createAssessmentHandler = function createAssessmentHandler(requestContext)
    {
        var deferred = Q.defer();

        try
        {
            var assessmentType = requestContext.payload.answerKey.assessmentType;
            ...
            var options =
                {
                    context:
                    {
                        userId: requestContext.headers['pi-id'],
                        courseId: requestContext.headers['course-id']
                    }
                };
    ...
    {
        createAssessmentHandler(requestContext)
     ...
     * 
     * @param  {Object}   answerKey  An object containing the data answerKey.
     * @param {Object} requestContext
     *      An object containing the data payload.
     *
     * @param {Function} callback
    ...
    {
        var answerKey = requestContext.payload.answerKey;
